"""
Shared AES Encryption Utility for Frame and Annotation Data
...
class CryptoService:
    ...
    def __init__(self, encryption_key: str = None):
        """Initialize the crypto service with the provided encryption key."""
        self._encryption_key = encryption_key or os.getenv("ENCRYPTION_KEY")
        self._validate_key()
        self._key = bytes.fromhex(self._encryption_key)
...
# Global crypto service instance - will be initialized when first accessed
...
def encrypt_bytes(data: bytes) -> bytes:
    ...
    """
    return get_crypto_service().encrypt_bytes(data)
...
def decrypt_bytes(encrypted_data: bytes) -> bytes:
    ...
    Returns:
        Decrypted raw bytes
    """
    return get_crypto_service().decrypt_bytes(encrypted_data)
...
def encrypt_string(text: str) -> bytes:
    ...
    Returns:
        Encrypted bytes
    """
    return get_crypto_service().encrypt_string(text)
...
def decrypt_string(encrypted_data: bytes) -> str:
    ...
    Returns:
        Decrypted string
    """
    return get_crypto_service().decrypt_string(encrypted_data)
