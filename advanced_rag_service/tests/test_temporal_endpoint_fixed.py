...
class MockTemporalRAGResponse:
    def __init__(self, linked_explanation, retrieved_context, explanation_confidence=0.8):
...
class TestTemporalRAGEndpoint:
    ...
    def test_temporal_rag_query_success(self, valid_request_payload, mock_temporal_response):
        ...
        with patch('main.advanced_rag.rag_service') as mock_service:
            ...
            # Verify response structure - should now include explanation_confidence
            ...
            assert isinstance(data["explanation_confidence"], (int, float))
            assert 0.0 <= data["explanation_confidence"] <= 1.0
    ...
    def test_response_format_consistency(self, mock_service, valid_request_payload, mock_temporal_response):
        ...
        # Verify response has exactly the expected fields - now includes explanation_confidence
        expected_fields = {"linked_explanation", "retrieved_context", "explanation_confidence"}
